...
# To avoid problems with eager executio we add the following line:
...
# Other data sets
filename_data = '../Data/Laminar_sampling/Fs_0.04_sampling.dat'
#filename_data = '../Data/Steady_sampling/Fs_32_maxCO30_sampling'
'''
laminar:
Fs_0.03_sampling.dat  Fs_0.04_sampling.dat  Fs_0.05_sampling.dat  Fs_0.06_sampling.dat  Fs_0.08_sampling.dat  Fs_0.15_sampling.dat  Fs_0.1_sampling.dat  Fs_0.2_sampling.dat
steady:
Fs_12_maxCO1_sampling   Fs_16_maxCO1_sampling   Fs_20_maxCO1_sampling   Fs_24_maxCO1_sampling   Fs_28_maxCO1_sampling   Fs_32_maxCO1_sampling   Fs_4_maxCO1_sampling   Fs_8_maxCO1_sampling
Fs_12_maxCO30_sampling  Fs_16_maxCO30_sampling  Fs_20_maxCO30_sampling  Fs_24_maxCO30_sampling  Fs_28_maxCO30_sampling  Fs_32_maxCO30_sampling  Fs_4_maxCO30_sampling  Fs_8_maxCO30_sampling
...
parser.add_argument('--LossModes',action="store_true",default=False,help="Use only modal equations during optimisation")
parser.add_argument('--BothEquations', action="store_true",default=False,help="Use modal and physical equations during optimisation")
...
parser.add_argument('--DataLocation', type=str, default = 'all', help="specify the location when using sparse data. Options are 'all' (default), 'inlet', 'cylinder_pitot', 'cylinder_only', 'pitot_only' ")
...
if args.BothEquations == True and args.LossModes == True:
    print("You want to use only 2 differents things for optimisation. Setting to use both equations. Stop if this is an error")
    args.LossModes = False
if args.SparseData and args.DataLocation == 'all':
    print("You want sparse data but you have did not specify location. I will use 'cylinder_pitot'.")
    args.DataLocation = 'cylinder_pitot'
...
d_c = 0.055 # diameter
r_c = d_c/2 # radius, used for the geometry
Lxmin = -4.*d_c
Lxmax = 8.*d_c
Lymin = -4.*d_c
Lymax = 4.*d_c
...
# normalize data:
x_c, y_c, r_c, Lxmin, Lxmax, Lymin, Lymax, omega_0, d_c = x_c/d_c, y_c/d_c, r_c/d_c, Lxmin/d_c, Lxmax/d_c, Lymin/d_c, Lymax/d_c, omega_0/Ur*d_c, d_c/d_c
Lx = Lxmax-Lxmin
Ly = Lymax-Lymin

pythonfile = os.path.basename(__file__)
repertoire = 'OutputPythonScript_Surrogates/ModalPINN_Ur_' + str(Ur) + '_Re_' + str(Re) + '_' + 'loc_' + str(args.DataLocation) + '_twoZones_' + str(args.TwoZonesSampling) + '_sparse_' + str(args.SparseData) + '_' + str(r)
...
print('Use both equations : ' + str(args.BothEquations))
...
print('Data location : ' + args.DataLocation)
...
def xbc5(s):
    '''
    Compute cylinders border x coordinate as a function of curvilinear abscissa s \in [0,1]    input : s (tf tensor, usually of shape [Nbc,1])
...
list_omega = np.asarray([k*omega_0 for k in range(Nmodes)]) #to use also not the first modes (when Re too high)


# Structure of each Neural Network that approximate a mode shape
#layers = [2,args.WidthLayer*Nmodes,args.WidthLayer*Nmodes,Nmodes] # runId =2
layers = [2,80,80,4]      # runId = 3,4,5  !!! Attention: need to change the function Neural_net in NN_func when using sin as activation !!!
...
Add explanation
'''
# Placeholders are like variables but you can initiaize them in the future
# Would use variable for trainable quantities such as weights or biases
# cf https://stackoverflow.com/questions/36693740/whats-the-difference-between-tf-placeholder-and-tf-variable
'''
End of explanation
'''
...
Nycl = 6
Ntimes = 300 # Number of timesteps in simulations data

# Placeholders for V_in (penalization of equations)
x_tf_int = tf.compat.v1.placeholder(name="x_int",dtype=tf.float32,shape=[None,1])
y_tf_int = tf.compat.v1.placeholder(name="y_int",dtype=tf.float32,shape=[None,1])
t_tf_int = tf.compat.v1.placeholder(name="t_int",dtype=tf.float32,shape=[None,1])

# Placeholders for general fitting data (especially dense data)
x_tf_mes = tf.compat.v1.placeholder(name="x_mes",dtype=tf.float32,shape=[None,1])
y_tf_mes = tf.compat.v1.placeholder(name="y_mes",dtype=tf.float32,shape=[None,1])
t_tf_mes = tf.compat.v1.placeholder(name="t_mes",dtype=tf.float32,shape=[None,1])
u_tf_mes = tf.compat.v1.placeholder(name="u_mes",dtype=tf.float32,shape=[None,1])
v_tf_mes = tf.compat.v1.placeholder(name="v_mes",dtype=tf.float32,shape=[None,1])
p_tf_mes = tf.compat.v1.placeholder(name="p_mes",dtype=tf.float32,shape=[None,1])
...
if args.DataLocation in ['cylinder_pitot', 'pitot_only', 'all']:    
# Placeholder for simulated pitot probe (uncomment the line if using pitot probes locations)
    x_tf_mes_pitot = tf.compat.v1.placeholder(name="x_pitot",dtype=tf.float32,shape=[None,1])
    y_tf_mes_pitot = tf.compat.v1.placeholder(name="y_pitot",dtype=tf.float32,shape=[None,1])
    t_tf_mes_pitot = tf.compat.v1.placeholder(name="t_pitot",dtype=tf.float32,shape=[None,1])
    u_tf_mes_pitot = tf.compat.v1.placeholder(name="u_pitot",dtype=tf.float32,shape=[None,1])
    v_tf_mes_pitot = tf.compat.v1.placeholder(name="v_pitot",dtype=tf.float32,shape=[None,1])
    p_tf_mes_pitot = tf.compat.v1.placeholder(name="p_pitot",dtype=tf.float32,shape=[None,1]) #  Not really used since only u and v are used at these locations for training
...
# WDelta_t_np_pitot doesn't even exist before this line and Delta_t_tf_pitot is never used again ...
...
if args.DataLocation in ['all', 'cylinder_pitot', 'cylinder_only']:
# Cylindre data for simulated pressure probe
    x_tf_mes_cyl = tf.compat.v1.placeholder(name="x_cyl",dtype=tf.float32,shape=[None,1])
    y_tf_mes_cyl = tf.compat.v1.placeholder(name="y_cyl",dtype=tf.float32,shape=[None,1])
    t_tf_mes_cyl = tf.compat.v1.placeholder(name="t_cyl",dtype=tf.float32,shape=[None,1])
    p_tf_mes_cyl = tf.compat.v1.placeholder(name="p_cyl",dtype=tf.float32,shape=[None,1])


# Border
s_tf = tf.compat.v1.placeholder(name="border",dtype=tf.float32,shape=[None,1])
one_s_tf = tf.compat.v1.placeholder(name="one_s",dtype=tf.float32,shape=[None,1])
...
def force_cylinder_real(t,x_cyl_r,y_cyl_r,s_cyl_r,u,v,p):
    '''
    t : tf.float32 tensor shape [Nt,1]  
    ----
    return
    ...
    # Reshape to [Ns,Nt]
    fx_tf_local_r2 = tf.reshape(fx_tf_local,[-1,Nt])
    fy_tf_local_r2 = tf.reshape(fy_tf_local,[-1,Nt])
...
def loss_int_mode(x,y):
    ...
    def customgrad(fgrad,xgrad):
        ...
        '''
        fgrad_xgrad =  [tf.complex(tf.gradients(tf.math.real(fgrad[:,:,k]), xgrad, grad_ys = one)[0],tf.gradients(tf.math.imag(fgrad[:,:,k]), xgrad, grad_ys = one)[0]) for k in range(Nmodes)]
    ...
    f_u_5a = [tf.reduce_sum(tf.convert_to_tensor([all_u[:,:,l]*tf.math.conj(all_u_x[:,:,l-k]) for l in range(k+1,Nmodes)]),axis=0) for k in range(Nmodes)]
    f_u_5a[-1] = f_u_5a[-2]*0.
    f_u += tf.transpose(tf.convert_to_tensor(f_u_5a), perm=[1,2,0])
    
    f_u_5b = [tf.reduce_sum(tf.convert_to_tensor([tf.math.conj(all_u[:,:,l-k])*all_u_x[:,:,l] for l in range(k+1,Nmodes)]),axis=0) for k in range(Nmodes)]
    f_u_5b[-1] = f_u_5b[-2]*0.
    f_u += tf.transpose(tf.convert_to_tensor(f_u_5b), perm=[1,2,0])

    f_u_5c = [tf.reduce_sum(tf.convert_to_tensor([all_v[:,:,l]*tf.math.conj(all_u_y[:,:,l-k]) for l in range(k+1,Nmodes)]),axis=0) for k in range(Nmodes)]
    f_u_5c[-1] = f_u_5c[-2]*0.
    f_u += tf.transpose(tf.convert_to_tensor(f_u_5c), perm=[1,2,0])
    
    f_u_5d = [tf.reduce_sum(tf.convert_to_tensor([tf.math.conj(all_v[:,:,l-k])*all_u_y[:,:,l] for l in range(k+1,Nmodes)]),axis=0) for k in range(Nmodes)]
    f_u_5d[-1] = f_u_5d[-2]*0.
    ...
    f_v_5a = [tf.reduce_sum(tf.convert_to_tensor([all_u[:,:,l]*tf.math.conj(all_v_x[:,:,l-k]) for l in range(k+1,Nmodes)]),axis=0) for k in range(Nmodes)]
    f_v_5a[-1] = f_v_5a[-2]*0.
    f_v += tf.transpose(tf.convert_to_tensor(f_v_5a), perm=[1,2,0])
    
    f_v_5b = [tf.reduce_sum(tf.convert_to_tensor([tf.math.conj(all_u[:,:,l-k])*all_v_x[:,:,l] for l in range(k+1,Nmodes)]),axis=0) for k in range(Nmodes)]
    f_v_5b[-1] = f_v_5b[-2]*0.  #quand k=N, k+1 > N
    f_v += tf.transpose(tf.convert_to_tensor(f_v_5b), perm=[1,2,0])

    f_v_5c = [tf.reduce_sum(tf.convert_to_tensor([all_v[:,:,l]*tf.math.conj(all_v_y[:,:,l-k]) for l in range(k+1,Nmodes)]),axis=0) for k in range(Nmodes)]
    f_v_5c[-1] = f_v_5c[-2]*0.
    f_v += tf.transpose(tf.convert_to_tensor(f_v_5c), perm=[1,2,0])
    
    f_v_5d = [tf.reduce_sum(tf.convert_to_tensor([tf.math.conj(all_v[:,:,l-k])*all_v_y[:,:,l] for l in range(k+1,Nmodes)]),axis=0) for k in range(Nmodes)]
    f_v_5d[-1] = f_v_5d[-2]*0.
...
if args.DataLocation in ['all', 'pitot_only', 'cylinder_pitot']:
# Wrap error on (u,v) measurements at simulated pitot probes locations
    ...
    Loss_mes_exp += Loss_mes_pitot

if args.DataLocation in ['all', 'cylinder_only', 'cylinder_pitot']:
...
elif args.BothEquations:                    # Physical and modal equations are used instead of modal equations
    Loss = Loss_int_time_wrap + Loss_int_mode_wrap + Loss_mes
else:
    Loss = Loss_int_time_wrap + Loss_mes    # Only physical equations are used
...
if args.SparseData:
    ...
    if args.DataLocation == 'cylinder_only':
        print('Loading Sparse Data on cylinder only')
        x_int,y_int,t_int,s_train,xmes_cyl,ymes_cyl,tmes_cyl,umes_cyl,vmes_cyl,pmes_cyl = ltd.training_dict(Nmes,Nint,Nbc,filename_data,geom,Tintmax=1e2,data_selection = args.DataLocation,desync=args.DesyncSparseData, multigrid=multigrid,Ngrid=Ngrid,stdNoise=stdNoise,method_int = IntSampling)
        Ncyl = len(xmes_cyl)
        Tmin = 400.
    ...
    if args.DataLocation == 'pitot_only':
        print('Loading Sparse Data on pitot probes only')
        x_int,y_int,t_int,s_train,xmes_pitot,ymes_pitot,tmes_pitot,umes_pitot,vmes_pitot,pmes_pitot,Delta_t_np_pitot = ltd.training_dict(Nmes,Nint,Nbc,filename_data,geom,Tintmax=1e2,data_selection = args.DataLocation,desync=args.DesyncSparseData, multigrid=multigrid,Ngrid=Ngrid,stdNoise=stdNoise,method_int = IntSampling)
        Npitot = len(xmes_pitot)
        Tmin = 400.
    ...
    if args.DataLocation == 'inlet':
        print('Loading Sparse Data on inlet')
        x_int, y_int, t_int, s_train, x_inlet, y_inlet, t_inlet, u_inlet, v_inlet, p_inlet = ltd.training_dict(Nmes,Nint,Nbc,filename_data,geom,Tintmax=1e2,data_selection = args.DataLocation,desync=args.DesyncSparseData, multigrid=multigrid,Ngrid=Ngrid,stdNoise=stdNoise,method_int = IntSampling)
        Ninlet = len(x_inlet)
        Tmin = 400.
        ...
        if multigrid:
            ...
            for k in range(Ngrid):
                tf_dict_temp = {x_tf_int : np.reshape(x_int[k],(Nint,1)),
                 ...
                 t_tf_int : np.reshape(t_int[k],(Nint,1)),
                 s_tf : np.reshape(s_train,(Nbc,1)),
                 x_tf_inlet : np.reshape(x_inlet,(Ninlet,1)),
                 y_tf_inlet : np.reshape(y_inlet,(Ninlet,1)),
                 t_tf_inlet : np.reshape(t_inlet,(Ninlet,1)),
                 u_tf_inlet : np.reshape(u_inlet,(Ninlet,1)),
                 v_tf_inlet : np.reshape(v_inlet,(Ninlet,1)),
                 p_tf_inlet : np.reshape(p_inlet,(Ninlet,1)),
                 }
                tf_dict.append(tf_dict_temp)
...
AdamTmax = Tmax-(t2-t0)
List_it_loss_Adam,List_it_loss_valid_Adam = nnf.model_train_Adam(opt_Adam,sess,Loss,liste_tf_dict=tf_dict,Nit=1e5,tolAdam=1e-5,it=it,itdisp=500,maxTime=AdamTmax,multigrid=multigrid,NgridTurn=NgridTurn,List_loss = True,tf_dict_valid=tf_dict_valid,loss_valid = Loss_dense_mes)
t3 = time.time()
...
for k in range(Nmodes):
    x_modes_np,y_modes_np,c_u_modes_np = sess.run([x_tf_int[:,0],y_tf_int[:,0],fluid_u(x_tf_int,y_tf_int)[0,:,k]],feed_dict=tf_dict_valid)
    X_modes.append(x_modes_np)
    Y_modes.append(y_modes_np)
    C_u_modes.append(c_u_modes_np)
    '''
    nnf.tf_plot_scatter_complex(,
                        sess,
                        title='u Mode '+str(k),
                        xlabel='$x$',ylabel='$y$',
...
for k in range(Nmodes):
    _, _, c_v_modes_np = sess.run([x_tf_int[:,0],y_tf_int[:,0],fluid_v(x_tf_int,y_tf_int)[0,:,k]],feed_dict=tf_dict_valid)
    C_v_modes.append(c_v_modes_np)
    '''
...
for k in range(Nmodes):
    _, _, c_p_modes_np = sess.run([x_tf_int[:,0],y_tf_int[:,0],fluid_p(x_tf_int,y_tf_int)[0,:,k]],feed_dict=tf_dict_valid)
    C_p_modes.append(c_p_modes_np)
    '''
...
filename_modes_shape = repertoire + '/Modes_shape.pickle'

Data_modes_shape = [X_modes, Y_modes, C_u_modes, C_v_modes, C_p_modes]
pckl_modes_shape = open(filename_modes_shape,'ab+')
pickle.dump(Data_modes_shape,pckl_modes_shape)
pckl_modes_shape.close()
print('Modes shape exported in '+repertoire)

# =============================================================================
# Comparison at a given timestep between modalPINN and simulations data
# =============================================================================
print('Saving comparison history...')

Re, Ur, times, nodes_X, nodes_Y, Us, Vs, Ps = ltd.read_cut_simulation_data(filename_data,geom)
timesteps = [int(len(times)/4),int(len(times)/2),int(3*len(times)/4),len(times)-1]
X_comp_u, X_comp_v, X_comp_p, Y_comp_u, Y_comp_v, Y_comp_p, U_comp, V_comp, P_comp, fluid_u_comp, fluid_v_comp, fluid_p_comp = [], [], [], [], [], [], [], [], [], [], [], []

for time in timesteps:
    tf_dict_compare_u = {
        x_tf_mes : np.reshape(nodes_X[0,:],(len(nodes_X[0,:]),1)),
        y_tf_mes : np.reshape(nodes_Y[0,:],(len(nodes_Y[0,:]),1)),
        t_tf_mes : np.reshape(times[time]*np.ones(len(nodes_X[0,:])),(len(nodes_Y[0,:]),1)),
        u_tf_mes : np.reshape(Us[time,:],(len(nodes_X[0,:]),1))
        }
    tf_dict_compare_v = {
        x_tf_mes : np.reshape(nodes_X[0,:],(len(nodes_X[0,:]),1)),
        y_tf_mes : np.reshape(nodes_Y[0,:],(len(nodes_Y[0,:]),1)),
        t_tf_mes : np.reshape(times[time]*np.ones(len(nodes_X[0,:])),(len(nodes_Y[0,:]),1)),
        v_tf_mes : np.reshape(Vs[time,:],(len(nodes_X[0,:]),1))
        }
    tf_dict_compare_p = {
        x_tf_mes : np.reshape(nodes_X[0,:],(len(nodes_X[0,:]),1)),
        y_tf_mes : np.reshape(nodes_Y[0,:],(len(nodes_Y[0,:]),1)),
        t_tf_mes : np.reshape(times[time]*np.ones(len(nodes_X[0,:])),(len(nodes_Y[0,:]),1)),
        p_tf_mes : np.reshape(Ps[time,:],(len(nodes_X[0,:]),1))
        }
    #suptitle='u difference at t = '+'{0:.2f}'.format(timestep)
    x_comp_np_u, y_comp_np_u, u_comp_np, fluid_u_comp_np = sess.run([x_tf_mes,y_tf_mes,u_tf_mes,fluid_u_t(x_tf_mes,y_tf_mes,t_tf_mes)],     feed_dict=tf_dict_compare_u)
    x_comp_np_v, y_comp_np_v, v_comp_np, fluid_v_comp_np = sess.run([x_tf_mes,y_tf_mes,v_tf_mes,fluid_v_t(x_tf_mes,y_tf_mes,t_tf_mes)],feed_dict=tf_dict_compare_v)
    x_comp_np_p, y_comp_np_p, p_comp_np, fluid_p_comp_np = sess.run([x_tf_mes,y_tf_mes,p_tf_mes,fluid_p_t(x_tf_mes,y_tf_mes,t_tf_mes)],feed_dict=tf_dict_compare_p)
    X_comp_u.append(x_comp_np_u)
    Y_comp_u.append(y_comp_np_u)
    X_comp_v.append(x_comp_np_v)
    Y_comp_v.append(y_comp_np_v)
    X_comp_p.append(x_comp_np_p)
    Y_comp_p.append(y_comp_np_p)
    U_comp.append(u_comp_np)
    V_comp.append(v_comp_np)
    P_comp.append(p_comp_np)
    fluid_u_comp.append(fluid_u_comp_np)
    fluid_v_comp.append(fluid_v_comp_np)
    fluid_p_comp.append(fluid_p_comp_np)


filename_comp_hist = repertoire + '/Comparison_history.pickle'

Data_comp_history = [X_comp_u, X_comp_v, X_comp_p, Y_comp_u, Y_comp_v, Y_comp_p, U_comp, V_comp, P_comp, fluid_u_comp, fluid_v_comp, fluid_p_comp, timesteps]
pckl_comp_hist = open(filename_comp_hist,'ab+')
pickle.dump(Data_comp_history,pckl_comp_hist)
pckl_comp_hist.close()
print('Comparison history exported in '+repertoire)
    ...
    #plt.savefig(repertoire+'/diff_u_t_'+'{0:.2f}'.format(timestep)+'.png')
'''
# =============================================================================
# Save forces to compare with real ones
# =============================================================================

print('Saving forces ...')
filename_force = repertoire + '/forces.pickle'
data_force = [np.array(force_cylinder_flatten(t_tf_mes)), np.array(force_cylinder_real(t_tf_mes,x_tf_mes,y_tf_mes,s_tf,u_tf_mes,v_tf_mes,p_tf_mes))]
pckl_force = open(filename_force, 'ab+')
pickle.dump(data_force, pckl_force)
pckl_force.close()

print('Forces exported')
'''
