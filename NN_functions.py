...
def NN_time_uv(x,y,t,weights,biases,geom,omega_0,trunc_mode=None):
    ...
    t_real = tf.math.real(tf.reduce_sum(t_parts,axis=0))  #compute the sum of elements (added comment)
...
def NN_time_p(x,y,t,weights,biases,omega_0,trunc_mode=None):
    ...
    parts = [out_NN[0,:,k]*tf.exp(k*omega_0*tf.complex(0.,t[:,0])) for k in range(Nmode)]
    t_parts = tf.convert_to_tensor(parts)
    t_real = tf.math.real(tf.reduce_sum(t_parts,axis=0))
...
def declare_LBFGS(loss,maxit=10000,maxfun=10000,ftol=1.0 * np.finfo(float).eps):
    optimizer = tf.contrib.opt.ScipyOptimizerInterface(loss, method = 'L-BFGS-B', options = 
                                                                          {'maxiter': maxit, #10000
                                                                           'maxfun': maxfun, #10000
                                                                           'maxcor': 50,
                                                                           'maxls': 50,
                                                                           'ftol' : ftol})
    '''Added to check if now it works
...
def plot_scatter(x_np,y_np,c_np,xlabel='x',ylabel='y',title=''):
    '''
    Parameters
    ----------
    x, y = things to plot
...
def plot_scatter_complex_4fig(x_np,y_np,c_np,xlabel='x',ylabel='y',title=''):
    '''
    Parameters
    ----------
    xlabel, ylabel, title : string to display on figure
...
def plot_scatter_complex(x_np,y_np,c_np,xlabel,ylabel,title):
    '''
    Parameters
    ----------
    xlabel, ylabel, title : string to display on figure
...
def plot_compare_3plot(x_np,y_np,c_np_1,c_np_2,xlabel='x',ylabel='y',title1='',title2='',suptitle=''):
    '''
    Return a figure with 3 subplots : c_np_1, c_np_2, log10((c_np_1-c_np_2)^2)
    '''
...
def plot_compare_9plot(x_mp, y_mp, c_mp_1, c_mp_2, x_p, y_p, c_p_1, c_p_2, x_m, y_m, c_m_1, c_m_2, xlabel='x', ylabel='y', colorbar_vit = 'Speed',
                       colorbar_press = 'Pressure', unit_v = 'm/s', unit_p = 'Pa', title1 ='Exact', title2 ='Modal and Physical', 
                       title3 = 'Physical', title4 = 'Modal', title5 = 'Square difference - log10', title = ''):

    err_mp = np.mean(np.log10(np.square(c_mp_1-c_mp_2)))
    err_m = np.mean(np.log10(np.square(c_m_1-c_m_2)))
    err_p = np.mean(np.log10(np.square(c_p_1-c_p_2)))

    fig = plt.figure(figsize=(24,15))
    plt.suptitle(title)
    plt.subplot(331)
    plt.scatter(x_mp,y_mp,c=c_mp_1,marker='.',s=1.)
    plt.axis('equal')
    plt.xlabel(xlabel + ' (m)')
    plt.ylabel(ylabel + ' (m)')
    plt.colorbar(label = colorbar_vit + ' (' + unit_v + ')')
    plt.title(title1)
    plt.subplot(332)
    plt.scatter(x_mp,y_mp,c=c_mp_2,marker='.',s=1.)
    plt.axis('equal')
    plt.xlabel(xlabel + ' (m)')
    plt.ylabel(ylabel + ' (m)')
    plt.colorbar(label = colorbar_vit + ' (' + unit_v + ')')
    plt.title(title2)
    plt.subplot(333)
    plt.scatter(x_mp,y_mp,c=np.log10(np.square(c_mp_1-c_mp_2)),marker='.',s=1.)
    plt.xlabel(xlabel + ' (m)')
    plt.ylabel(ylabel + ' (m)')
    plt.axis('equal')
    plt.colorbar(label = 'Error ((' + unit_v + ')^2)')
    plt.title(title5 + '  (mean err: '+str(err_mp)+')')
    
    
    plt.subplot(334)
    plt.scatter(x_p,y_p,c=c_p_1,marker='.',s=1.)
    plt.axis('equal')
    plt.xlabel(xlabel + ' (m)')
    plt.ylabel(ylabel + ' (m)')
    plt.colorbar(label = colorbar_vit + ' (' + unit_v + ')')
    plt.title(title1)
    plt.subplot(335)
    plt.scatter(x_p,y_p,c=c_p_2,marker='.',s=1.)
    plt.axis('equal')
    plt.xlabel(xlabel + ' (m)')
    plt.ylabel(ylabel + ' (m)')
    plt.colorbar(label = colorbar_vit + ' (' + unit_v + ')')
    plt.title(title3)
    plt.subplot(336)
    plt.scatter(x_p,y_p,c=np.log10(np.square(c_p_1-c_p_2)),marker='.',s=1.)
    plt.axis('equal')
    plt.xlabel(xlabel + ' (m)')
    plt.ylabel(ylabel + ' (m)')
    plt.colorbar(label = 'Error ((' + unit_v + ')^2)')
    plt.title(title5 + '  (mean err: '+str(err_p)+')')

    
    plt.subplot(337)
    plt.scatter(x_m,y_m,c=c_m_1,marker='.',s=1.)
    plt.axis('equal')
    plt.xlabel(xlabel + ' (m)')
    plt.ylabel(ylabel + ' (m)')
    plt.colorbar(label = colorbar_press + ' (' + unit_p + ')')
    plt.title(title1)
    plt.subplot(338)
    plt.scatter(x_m,y_m,c=c_m_2,marker='.',s=1.)
    plt.axis('equal')
    plt.xlabel(xlabel + ' (m)')
    plt.ylabel(ylabel + ' (m)')
    plt.colorbar(label = colorbar_press + ' (' + unit_p + ')')
    plt.title(title4)
    plt.subplot(339)
    plt.scatter(x_m,y_m,c=np.log10(np.square(c_m_1-c_m_2)),marker='.',s=1.)
    plt.axis('equal')
    plt.xlabel(xlabel + ' (m)')
    plt.ylabel(ylabel + ' (m)')
    plt.colorbar(label = 'Error ((' + unit_p + ')^2)')
    plt.title(title5 + '  (mean err: '+str(err_m)+')')
    fig.tight_layout()
