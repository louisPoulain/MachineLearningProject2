...
filename_data = '../Steady_sampling/Fs_4_maxCO1_sampling.dat'
...
repertoire = 'OutputPythonScript_Surrogates/ClassicPINN_'+ d.strftime("%Y_%m_%d-%H_%M_%S") + '__' +str(r)
...
x_tf_mes_pitot = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,1])
y_tf_mes_pitot = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,1])
t_tf_mes_pitot = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,1])
u_tf_mes_pitot = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,1])
v_tf_mes_pitot = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,1])
p_tf_mes_pitot = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,1]) #  Not really used since only u and v are used at these locations for training
...
print(force_cylinder_flatten(t_tf_int))
